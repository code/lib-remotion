...
The official [Electron template](/templates/electron) leaves this disabled by default. Uncomment the browser staging block in `forge.config.ts` if you want packaged renders to work completely offline. This will increase the final app size significantly.
